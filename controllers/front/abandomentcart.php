...
class IcommktconnectorAbandomentcartModuleFrontController extends ModuleFrontController
{
        ...
        if(!empty($loaded_cart->id)){
            ...
            /*Si es usuario*/
                ...
                if ($loaded_cart->id_customer == $customer->id){
                    ...
                    $this->redirect = $this->context->link->getPageLink('authentication', null, null, array('back' => $this->context->link->getPageLink('order')));
                }
            }
    ...
    public function connectApi(){
        $days_to_abandon = !empty(Configuration::get('ICOMMKT_DAYS_TO_ABANDON', null)) ? Configuration::get('ICOMMKT_DAYS_TO_ABANDON', null) : '1';
...
//            $query = 'SELECT result.*
//                FROM (SELECT c.id_customer, c.id_cart, cus.email, cus.firstname, c.date_upd FROM `'._DB_PREFIX_.'cart` c
